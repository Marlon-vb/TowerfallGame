...
final class GameScene: SKScene {
    ...
    private var arrowTextures: [UInt8: SKTexture] = [:]
    ...
    private var worldName: String { MapThemes.theme(for: mapId).world }

    // Loads a texture from the bundled Sprites folder with pixel filtering.
    private func pixelTexture(_ name: String, subdirectory: String) -> SKTexture? {
    ...
    // Full-bleed 16-bit world background (falls back to the flat theme color).
    private func buildBackground() {
        guard let tex = pixelTexture(worldName, subdirectory: "Sprites/backgrounds") else { return }
        let node = SKSpriteNode(texture: tex,
                                size: CGSize(width: CGFloat(worldWidthPx), height: CGFloat(worldHeightPx)))
    ...
    // Loads the 16 edge-mask variants for this map's world tileset (generated
    // by tools/generate_pixel_art.py). Returns nil if the set is missing, in
    // which case tiles fall back to flat colored squares.
    private func loadTileset() -> [SKTexture]? {
        var textures: [SKTexture] = []
        for mask in 0..<16 {
            guard let tex = pixelTexture("t\(mask)", subdirectory: "Sprites/tiles/\(worldName)") else { return nil }
            textures.append(tex)
        }
        return textures
    }
    ...
    private func buildEntities() {
        ...
        // Pixel arrow sprites, one texture per ArrowKind (point right at zero
        // rotation; render rotates them).
        let kindFiles: [(UInt8, String)] = [(0, "arrow"), (1, "arrow_bomb"), (2, "arrow_laser"),
                                            (3, "arrow_drill"), (4, "arrow_feather")]
        for (kind, file) in kindFiles {
            if let tex = pixelTexture(file, subdirectory: "Sprites/fx") {
                arrowTextures[kind] = tex
            }
        }
        for _ in 0..<state.arrows.count {
            let node: SKSpriteNode
            if let tex = arrowTextures[0] {
        ...
        // Treasure chest (one per round).
        let chest: SKSpriteNode
        if let tex = pixelTexture("chest", subdirectory: "Sprites/fx") {
            chest = SKSpriteNode(texture: tex, size: CGSize(width: 16, height: 14))
        } else {
            chest = SKSpriteNode(color: .brown, size: CGSize(width: 16, height: 14))
        }
        chest.isHidden = true
        chest.zPosition = 5
    ...
    private func detectEvents(previous: GameState, current: GameState) {
        ...
        for i in 0..<arrowCount {
            ...
            // A bomb arrow that vanished detonated (tile impact or direct hit;
            // a dash-catch "defusing" it gets the same small pop, which reads
            // fine). Big render-only boom at its last position.
            if previous.arrows[i].active && !current.arrows[i].active
                && previous.arrows[i].arrowKind == .bomb {
                let pos = skPoint(previous.arrows[i].pos.x.toFloat, previous.arrows[i].pos.y.toFloat)
                spawnExplosion(at: pos)
            }
        }

        // Treasure chest events.
        if !previous.chest.active && current.chest.active, let chest = chestNode {
            AudioManager.shared.play("ui", on: self)
            chest.removeAllActions()
            chest.setScale(0.2)
            chest.run(.sequence([.scale(to: 1.15, duration: 0.12), .scale(to: 1.0, duration: 0.08)]))
        }
        if previous.chest.active && !current.chest.active && current.phase == .playing {
            AudioManager.shared.play("ui", on: self)
            spawnDust(at: skPoint(previous.chest.pos.x.toFloat, previous.chest.pos.y.toFloat))
        }
    }

    // Render-only bomb blast: flash ring + ember particles + a hard shake.
    private func spawnExplosion(at point: CGPoint) {
        ...
        ring.position = point
        ring.strokeColor = SKColor(red: 1.0, green: 0.75, blue: 0.3, alpha: 1)
        ring.lineWidth = 3
        ring.fillColor = SKColor(red: 1.0, green: 0.55, blue: 0.15, alpha: 0.5)
        ring.zPosition = 45
        world.addChild(ring)
        ring.run(.sequence([
            .group([.scale(to: 7.0, duration: 0.22), .fadeOut(withDuration: 0.22)]),
            .removeFromParent(),
        ]))
        for i in 0..<10 {
            let ember = SKSpriteNode(color: i % 2 == 0
                                     ? SKColor(red: 1.0, green: 0.7, blue: 0.25, alpha: 1)
                                     : SKColor(red: 0.9, green: 0.35, blue: 0.1, alpha: 1),
                                     size: CGSize(width: 3, height: 3))
            ember.position = point
            ember.zPosition = 46
            world.addChild(ember)
            let angle = CGFloat(i) / 10 * .pi * 2
            let dist: CGFloat = 22 + CGFloat(i % 3) * 8
            ember.run(.sequence([
                .group([
                    .move(by: CGVector(dx: cos(angle) * dist, dy: sin(angle) * dist), duration: 0.3),
    ...
    private func renderInterpolated(alpha: Float, dt: TimeInterval) {
        ...
        for a in 0..<arrowNodes.count where a < current.arrows.count {
            ...
            if let tex = arrowTextures[cur.kind] {
                node.texture = tex
            }
        ...
        if let chest = chestNode {
            chest.isHidden = !current.chest.active
            if current.chest.active {
                chest.position = skPoint(current.chest.pos.x.toFloat, current.chest.pos.y.toFloat)
            }
        }

        let me = driver.localPlayer
        if me < current.players.count {
            let p = current.players[me]
            var text = "Arrows: \(p.arrows)/\(config.startingArrows)"
            if p.specialCount > 0 {
                let names: [UInt8: String] = [1: "BOMB", 2: "LASER", 3: "DRILL", 4: "FEATHER"]
                text += "   \(names[p.specialKind] ?? "?") x\(p.specialCount)"
            }
            hud.text = text
        }
